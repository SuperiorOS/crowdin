...
# directly to SuperiorOS's Gerrit.
#
# Copyright (C) 2014-2016 The CyanogenMod Project
# Copyright (C) 2017-2019 The LineageOS Project
# Copyright (C) 2020 The Superior OS Project
#
...
def submit_gerrit(branch, username):
    ...
    cmd = ['ssh', '-p', '29418',
        '{}@gerrit.superioros.org'.format(username),
        'gerrit', 'query',
        'status:open',
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Synchronising SuperiorOS's translations with Crowdin")
    parser.add_argument('-u', '--username', help='Gerrit username')
    parser.add_argument('-b', '--branch', help='SuperiorOSs branch',
                        required=True)
...
def main():
    ...
    base_path_env = 'CROWDIN_BASE_PATH_%s' % base_path_branch_suffix
    ...
    xml_snippet = load_xml(x='%s/manifest/snippets/superior.xml' % base_path)
